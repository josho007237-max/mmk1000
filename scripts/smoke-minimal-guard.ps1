param(
  [string]$Base = "http://127.0.0.1:4100",
...
$ApiBase = if ($Base -match '/api$') { $Base } else { "$Base/api" }
$HealthUrl = if ($Base -match '/api$') { "$Base/health" } else { "$Base/api/health" }
...
function Ensure-ApiReachable {
  ...
  $raw = (& curl.exe -sS --max-time 15 $HealthUrl -w "`n__HTTP__:%{http_code}" 2>$null) -join "`n"
  $parts = $raw -split "__HTTP__:"
  if ($parts.Length -lt 2) {
    Write-Host "MMK1000 API health check failed: $HealthUrl (status=0)"
    exit 1
  }
  $statusRaw = [string]$parts[1]
  $status = [int]($statusRaw.Trim())
  if ($status -ne 200) {
    Write-Host "MMK1000 API health check failed: $HealthUrl (status=$status)"
    exit 1
  }
}

function Check-Routes {
  $routesUrl = "$ApiBase/routes"
  $raw = (& curl.exe -sS --max-time 10 $routesUrl -H "x-admin-key: $AdminKey" -w "`n__HTTP__:%{http_code}" 2>$null) -join "`n"
  $parts = $raw -split "__HTTP__:"
  if ($parts.Length -lt 2) { throw "routes_failed status=0 url=$routesUrl" }
  $statusRaw = [string]$parts[1]
  $status = [int]($statusRaw.Trim())
  if ($status -eq 404) {
    Write-Host "[skip routes] NODE_ENV=production ทำให้ route dev-only หายได้"
    return
  }
  if ($status -ne 200) {
    $body = ""
    if ($parts.Length -gt 0 -and $null -ne $parts[0]) { $body = ([string]$parts[0]).Trim() }
    throw "routes_failed status=$status url=$routesUrl body=$body"
  }
  Write-Host "[routes] PASS"
}

function Run-Case1 {
  Ensure-ApiReachable
  Write-Host "[case1] create->approve->send with cfg missing should not become sent"
  $create = Invoke-Api "POST" "$ApiBase/withdraw/create" @{
    type = "bank"
    amount = 1
  ...
  $approve = Invoke-Api "POST" "$ApiBase/withdraw/$id/approve" @{}
  Assert-True ($approve.status -eq 200) "case1_approve_failed status=$($approve.status) body=$($approve.raw)"

  $send = Invoke-Api "POST" "$ApiBase/withdraw/$id/send" @{}
  ...
  $queue = Invoke-Api "GET" "$ApiBase/withdraw/queue"
