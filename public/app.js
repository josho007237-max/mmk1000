...
function ensureSafeModeBanner() {
  ...
  banner.textContent = "Use REAL keys only behind Cloudflare Access";
...
function headers(extra = {}) {
  ...
  if (!ak) throw new Error("Missing ADMIN KEY");
...
async function sendNow(id, btn) {
  ...
  if (isRealMode()) {
    const typed = prompt("REAL MODE: type SEND to confirm");
    if ((typed || "").trim() !== "SEND") {
      setBanner("info", "ยกเลิกการโอน (ยืนยันไม่ถูกต้อง)");
      return;
    }
  }
  ...
  items.forEach(it => {
    ...
    if (tbody) {
      ...
      tr.innerHTML = `
        ...
        <td>${destToText(it)}</td>
        <td>
          <button class="btn-approve" ${canApprove ? "" : "disabled"} onclick="approve('${it.id}', this)">Approve</button>
          <button class="btn-send" ${canSend ? "" : "disabled"} onclick="sendNow('${it.id}', this)">Send</button>
        </td>
      `;
...
async function decodeQrIntoWithdraw(btn) {
  ...
  setButtonLoading(btn, true, "Decoding...");
